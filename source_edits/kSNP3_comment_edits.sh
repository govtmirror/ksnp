...
#? Add all items in working directory to a paths list???
# Variables were captured about in getops
if ($?dir) then 
...
# when looking at an assembled genome.  median or midpoint should be 1.  mean likely just above 1 at ~ 1.0213.  Minimum coverage is likely to also be 1, so likely all reads are going to be kept.
# I'm assuming this is only for raw reads
date
foreach f (fsplit*[0-9])
...
foreach f (fsplit*)
    ...
    mkdir Dir.$f
    cd Dir.$f
# I think this is just binning based on 1st 4 base possible combinations to run parallel later
    $kSNP/subset_mers3 ../kmers.$f
    printf "" >! cmds_remove_conflicting 
    foreach subset (*.mers)
# Just makes file to paths of binned files
	echo "$kSNP/delete_allele_conflicts3 $subset" >> cmds_remove_conflicting
    end
#Finding conflicts, parallelly, using the binned files
...
# All kmers from all genomes are be combined
echo "Merged sorted kmer files and remove duplicates"
date
$kSNP/subset_mer_list3 > ! mer_list
printf "" >! cmds_sort
# make a file with the sort command for each 4base.mer file listed
foreach subset (`cat mer_list`)
# These subsets are the 4 base combo files
...
################################
################################
################################ I think this portion is ran only if SNPs_all is called, aka... adding new genome to already done analysis
...
# Not adding new.  Normal SNP finding analysis
if (! $?SNPs_all  ) then 
    ...
    foreach subset (`cat mer_list`)
# Looks like just making a file list again
...
# going into the Dir.$f (f=fsplit0) is genome specific
date
echo "Finding allele in each genome"
printf "" >! cmds_find_allele
foreach f (fsplit*)
    foreach subset (`cat mer_list`)
# going into the Dir.$f (f=fsplit0) is genome specific and thus allowing the match
	echo "$kSNP/find_allele3 SNP_loci.$subset.fasta  Dir.$f/$subset.conflictsDeleted $f > Dir.$f/SNPs.$subset" >> cmds_find_allele
# example:
# /usr/local/kSNP3/find_allele3 SNP_loci.AGCC.mers.fasta  Dir.fsplit0/AGCC.mers.conflictsDeleted fsplit0 > Dir.fsplit0/SNPs.AGCC.mers
    end
end
$kSNP/parallel_commands3 $num_cpus cmds_find_allele
# the output SNP.$subset is:
# AAAAACTCT.GCAGCGCAT     C       AAAAACTCTCGCAGCGCAT     fsplit0
# AAAACATCG.CGCGCTCGG     T       AAAACATCGTCGCGCTCGG     fsplit0
# etc...
foreach f (fsplit*)
# each Dir.$f gets a "SNPs" file merging all subsets together
...
# all SNP from all genomes, marked as such:
# all_SNPs_unsorted
# CCGCGACCT.ATGACGGCC     A       x       16-014019-008-B3-ver
# CCGCGACGG.CTGCTGTCG     C       x       16-014019-008-B3-ver
# CCGCGACTC.GTCGGCGGC     G       x       16-014019-008-B3-ver
# "x" is position if ammotated genome
...
# notice SNPs are numbered
# SNPs_all (This file is saved with each analysis)
# 0       AAAAACTCT.GCAGCGCAT     C       x       16-016565-dassie-fcc47-ver
# 0       AAAAACTCT.GCAGCGCAT     C       x       16-016565-dassie-fcc48-ver
# 0       AAAAACTCT.GCAGCGCAT     T       x       16-016565-dassie-fcc49-ver
# 0       AAAAACTCT.GCAGCGCAT     T       x       16-016565-dassie-fcc50-ver
# 0       AAAAACTCT.GCAGCGCAT     T       x       16-2185-11
# 0       AAAAACTCT.GCAGCGCAT     T       x       16-2376
# if annotated..
# 0       AAAAACTCT.GCAGCGCAT     T       4374070 F       Mtb_H37Rv-REFSEQ        gi|561108321|ref|NC_018143.2|

# 1       AAAACATCG.CGCGCTCGG     C       x       16-014019-001-B22-ver
# 1       AAAACATCG.CGCGCTCGG     C       x       16-014019-007-B8-ver
# 1       AAAACATCG.CGCGCTCGG     C       x       16-014019-008-B3-ver
# etc...
